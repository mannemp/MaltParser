 ...
 */
public class MPEvaluator {

	/**
	 * @param args
	 */
//	public static void readProperties(String[] args) {
	public MPEvaluator(String[] args) {
		...
		for(int i=0;i<args.length;i++)
		{
			...
			{
				MPEvaluator.normalize = true;
				String n = args[++i];
				if(n.compareTo("sum") == 0)
					MPEvaluator.nml = new SumNormalizor();
				else if(n.compareTo("zscore") == 0)
					MPEvaluator.nml = new ZScoreNormalizor();
				else if(n.compareTo("linear") == 0)
					MPEvaluator.nml = new LinearNormalizer();
				else
				{
		...
		MPEvaluator e = new MPEvaluator(rType2[rankerType], trainMetric, testMetric);
		if(trainFile.compareTo("")!=0)
		{
			...
			System.out.println("Feature normalization: " + ((MPEvaluator.normalize)?MPEvaluator.nml.name():"No"));
	...
	public static boolean useSparseRepresentation = true;
	...
	public MPEvaluator(RANKER_TYPE rType, METRIC trainMetric, METRIC testMetric)
	{
		this.type = rType;
		trainScorer = mFact.createScorer(trainMetric);
		testScorer = mFact.createScorer(testMetric);
	...
	public MPEvaluator(RANKER_TYPE rType, METRIC trainMetric, int trainK, METRIC testMetric, int testK)
	{
		this.type = rType;
	...
	public MPEvaluator(RANKER_TYPE rType, METRIC trainMetric, METRIC testMetric, int k)
	{
		this.type = rType;
	...
	public MPEvaluator(RANKER_TYPE rType, METRIC metric, int k)
	{
		this.type = rType;
	...
	public MPEvaluator(RANKER_TYPE rType, String trainMetric, String testMetric)
	{
		this.type = rType;
		trainScorer = mFact.createScorer(trainMetric);
		testScorer = mFact.createScorer(testMetric);
	 ...
	 */
	public void evaluate(List<RankList> train, List<RankList> validation, List<RankList> test, String featureDefFile)
	{
	 ...
	 */
	public void score(String modelFile, List<RankList> test, String outputFile)
	{
		Ranker ranker = rFact.loadRanker(modelFile);
		int[] features = ranker.getFeatures();
//		List<RankList> test = readInput(testFile);
