...
public class DGraphAddressFunction extends AddressFunction {
	public enum DGraphSubFunction {
		HEAD, LDEP, RDEP, RDEP2, LSIB, RSIB, PRED, SUCC, ANC, PANC, LDESC, PLDESC, RDESC, PRDESC, R2DEP, L2DEP, HEAD2
	};
...
//				a.getAddressClass().asSubclass(org.maltparser.core.syntaxgraph.node.DependencyNode.class);
				...
				} else if (subFunction == DGraphSubFunction.L2DEP && node.getLeftmostDependent() != null) {
					address.setAddress(node.getLeftmostDependent().getRightSibling());
				} else if (subFunction == DGraphSubFunction.R2DEP && node.getRightmostDependent() != null) {
					address.setAddress(node.getRightmostDependent().getLeftSibling());
				} else if (subFunction == DGraphSubFunction.HEAD2 && !node.isRoot() && node.hasHead()) {
					address.setAddress(node.getHead().getHead());
				} else {
					address.setAddress(null);
				}
