...
public class DependencyGraph extends Sentence implements DependencyStructure { 
	...
	public int getLoss(DependencyGraph gold) throws MaltChainedException
	{
		int loss = 0;
		for(Edge sEdge: graphEdges)
		{
			TokenNode gTokenNode = gold.getTokenNode(sEdge.getTarget().getIndex());
			if(gTokenNode.getHead().getIndex() != sEdge.getSource().getIndex())
				loss++;
		}
		return loss;
	}
	
	public int getLLoss(DependencyGraph gold) throws MaltChainedException
	{
		int loss = 0;
		for(Edge sEdge: graphEdges)
		{
			TokenNode gTokenNode = gold.getTokenNode(sEdge.getTarget().getIndex());
			Edge gEdge = gTokenNode.getHeadEdge(); 
			
			Iterator<SymbolTable> giter = gEdge.getLabelTypes().iterator();
			String glab = "";
			if(giter.hasNext())
				glab = gEdge.getLabelSymbol(giter.next());
			String slab = glab;
			Iterator<SymbolTable> siter = sEdge.getLabelSet() != null ? sEdge.getLabelTypes().iterator() : null;
			if(siter != null && siter.hasNext())
				slab = sEdge.getLabelSymbol(siter.next());
			
			if(gEdge.getSource().getIndex() != sEdge.getSource().getIndex())
				loss++;
			else if (!slab.equals(glab))
				loss++;
