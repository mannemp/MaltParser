	...
	public void initialize(FlowChartInstance flowChartinstance, ChartItemSpecification chartItemSpecification) throws MaltChainedException {
		...
		if (taskName.equals("versioning")) {
			...
			if (configDir.getCreatedByMaltParserVersion() == null) {
				...
				SystemLogger.logger().info("WARNING:The parser model '"+ configDirName+ ".mco' is created by MaltParser "+configDir.getCreatedByMaltParserVersion()+".\n");
				SystemLogger.logger().info("WARNING:You have to re-train the parser model to be able to parse with current version of MaltParser.\n");
				SystemLogger.logger().info("WARNING:Proceeding with care\n");
//				System.exit(1);
			}
	...
	public int preprocess(int signal) throws MaltChainedException {
		...
		} else if (taskName.equals("loadsymboltablesfromfile")) {
			String filename = configDir.getConfigDirectory().getAbsoluteFile()+".psym";
			SystemLogger.logger().warn("Loading symboltables from "+filename+"\n");
			configDir.getSymbolTables().load(filename, outCharSet);
//			configDir.getSymbolTables().load(configDir.getInputStreamReaderFromConfigFileEntry("symboltables.sym",inCharSet));
		}
		return signal;
	}
	...
	public int postprocess(int signal) throws MaltChainedException {
		...
		else if (taskName.equals("savesymboltablestofile")) {
//			String filename = OptionManager.instance().getOptionValue(getOptionContainerIndex(), "pruneandscore", "symboltablesfile").toString();
			String filename = configDir.getConfigDirectory().getAbsoluteFile()+".psym";
			SystemLogger.logger().warn("Saving symboltables to "+filename+"\n");
			configDir.getSymbolTables().save(filename, outCharSet);
		}
		return signal;
	}
