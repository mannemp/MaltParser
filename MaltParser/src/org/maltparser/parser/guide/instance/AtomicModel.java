...
import org.maltparser.ml.lib.LibPruneAndScore;
...
public class AtomicModel implements InstanceModel {
	...
	public void addInstance(SingleDecision decision, DependencyStructure goldGraph) throws MaltChainedException {
		try {
			method.addInstance(decision, featureVector,goldGraph);
		} catch (NullPointerException e) {
			throw new GuideException("The learner cannot be found. ", e);
		}
	}
	...
	public void addPruneInstance(SingleDecision decision) throws MaltChainedException {
		try {
//			method.addRankInstance(decision, featureVector);
			if(parent.getGuide().getConfiguration() instanceof PruneAndScore)
			{
				method.addPruneInstance(decision, featureVector);
//				((PruneAndScore)parent.getGuide().getConfiguration()).addPruneInstance(decision,featureVector);
			}
		} catch (NullPointerException e) {
			throw new GuideException("The learner cannot be found. ", e);
		}
...
//			}
	...
	public void initMethod() throws MaltChainedException {
		...
		arguments[0] = this;
		if (getGuide().getConfiguration() instanceof PruneAndScore){
				arguments[1] = getGuide().getConfiguration().getMode();
			...
			arguments[1] = LearningMethod.CLASSIFY;
				...
				arguments[1] = LearningMethod.LEARN;
		}
