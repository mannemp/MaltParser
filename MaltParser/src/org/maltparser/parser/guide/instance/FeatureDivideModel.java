...
public class FeatureDivideModel implements InstanceModel {
	...
	public void addInstance(SingleDecision decision, DependencyStructure goldGraph) throws MaltChainedException {
		...
		if (divideModels != null) { 
			...
			divideModels.get(((SingleFeatureValue)divideFeature.getFeatureValue()).getIndexCode()).addInstance(decision, goldGraph);
		} else {
			throw new GuideException("The feature divide models cannot be found. ");
		}
	}
	...
	public void addPruneInstance(SingleDecision decision) throws MaltChainedException {
		...
		if (divideModels != null) { 
			...
			divideModels.get(((SingleFeatureValue)divideFeature.getFeatureValue()).getIndexCode()).addPruneInstance(decision);
		} else {
			throw new GuideException("The feature divide models cannot be found. ");
		}
	}
...
//				divideFeature.updateCardinality();
				for (Integer index : divideModels.keySet()) {
					divideModels.get(index).saveModel(preSuffix);
				}
				final TreeSet<Integer> removeSet = new TreeSet<Integer>();
				for (Integer index : divideModels.keySet()) {
					if (divideModels.get(index).getFrequency() <= divideThreshold) {
				...
				masterModel.saveModel(preSuffix);
