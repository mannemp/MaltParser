...
public class SeqDecisionModel implements DecisionModel {
	...
	public void saveModel(String preSuffix) throws MaltChainedException {
		...
		if (instanceModel != null) {
			instanceModel.saveModel(preSuffix);
		}
		if (nextDecisionModel != null) {
			nextDecisionModel.saveModel(preSuffix);
		}
	}
	...
	public void addInstance(GuideDecision decision, DependencyStructure goldGraph) throws MaltChainedException {
		...
		instanceModel.addInstance(singleDecision, goldGraph);
		if (singleDecision.continueWithNextDecision() && decisionIndex+1 < decision.numberOfDecisions()) {
			if (nextDecisionModel == null) {
				initNextDecisionModel(((MultipleDecision)decision).getSingleDecision(decisionIndex+1), branchedDecisionSymbols);
			}
			nextDecisionModel.addInstance(decision, goldGraph);
		}
	}
	...
	public void addPruneInstance(GuideDecision decision) throws MaltChainedException {
		...
		instanceModel.addPruneInstance(singleDecision);
		if (singleDecision.continueWithNextDecision() && decisionIndex+1 < decision.numberOfDecisions()) {
			if (nextDecisionModel == null) {
				initNextDecisionModel(((MultipleDecision)decision).getSingleDecision(decisionIndex+1), branchedDecisionSymbols);
			}
			nextDecisionModel.addPruneInstance(decision);
		}
	}
