...
public class BatchTrainer extends Trainer {
	...
	public DependencyStructure pasParse(DependencyStructure goldDependencyGraph, DependencyStructure parseDependencyGraph) throws MaltChainedException {
		parserState.clear();
		parserState.initialize(parseDependencyGraph);
		currentParserConfiguration = parserState.getConfiguration();
		if(currentParserConfiguration instanceof NivreConfig)
			((NivreConfig) currentParserConfiguration).setGoldArcs(goldDependencyGraph);
		parseCount++;
		if (diagnostics == true) {
			writeToDiaFile(parseCount + "");
		}
		...
		while (!parserState.isTerminalState()) {
			...
			if (action == null) {
				action = oracleGuide.predict(goldDependencyGraph, currentParserConfiguration);
				try {
					classifierGuide.addInstance((GuideDecision)action, goldDependencyGraph);
					
					/*if(getManager().getMode() == PruneAndScore.SLEARN)
					{
						classifierGuide.addInstance((GuideDecision)action);
					}
					else if(getManager() instanceof PruneAndScore && 
							getManager().getMode() == PruneAndScore.PLEARN)
					{
						classifierGuide.addPruneInstance((GuideDecision)action);
//						((PruneAndScore)getManager()).getPruner().train((GuideDecision)action);	
					}*/
