...
public class KBestList {
	...
	// Only used in PruneAndScore
	public ArrayList<Candidate> getKBestList()
	{
		kBestList = new ArrayList<Candidate>();
		return kBestList;
	}
	...
	public void bringToTop(int n)
	{
		Candidate cand0 = kBestList.get(0);
		kBestList.set(0, kBestList.get(n));
		kBestList.set(n, cand0);
		return;
	}
...
//			}
	 ...
	 * @throws MaltChainedException
	 */
	public void add(int actionCode, double score) throws MaltChainedException {
		if (k != -1 && addCandidateIndex >= k) { return; }
		if (addCandidateIndex >= kBestList.size()) { kBestList.add(new ScoredCandidate()); }
		kBestList.get(addCandidateIndex).setActionCode(actionCode);
		((ScoredCandidate)kBestList.get(addCandidateIndex)).setScore((float)score);
		if (addCandidateIndex == 0) {
//			if (decision instanceof SingleDecision) {
//				((SingleDecision)decision).addDecision(actionCode);
//			}
	...
	public void addList(double[][] predictionList) throws MaltChainedException {
		int n = (k != -1 && k <= predictionList[0].length-1)?k:predictionList[0].length - 1;
		for (int i=0; i<n; i++) {
			add((int)predictionList[0][i],predictionList[1][i]);
		}	
	}
...
//		}
	...
	public Candidate getUnScoredCandidate(int i) {
		if (i >= kBestList.size()) {
			return null;
		}
		return kBestList.get(i);
	}
